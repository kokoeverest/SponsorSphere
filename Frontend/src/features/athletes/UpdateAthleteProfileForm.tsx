...
import { UpdateAthleteProfileFormInput } from "./abstract";
import { UpdateAthleteProfileSchema } from "./schemas";
...
const UpdateAthleteProfileForm: React.FC = () =>
{
    const { role, userType } = useContext( AuthContext );
    const navigate = useNavigate();
    const location = useLocation();
    const queryClient = useQueryClient();
    const user = location.state as AthleteDto;
    const profilePage = `/athletes/${ user.id }`;

    const [ selectedCountry, setSelectedCountry ] = useState( user.country );
    const [ selectedSport, setSelectedSport ] = useState( role === 'Athlete' ? user.sport : null );
    ...
    const mutation = useMutation( {
        mutationFn: athleteApi.updateAthlete,
        onSuccess: () =>
        {
    ...
    const onSubmitHandler: SubmitHandler<UpdateAthleteProfileFormInput> = async ( data ) =>
    {
    ...
    const { register, handleSubmit, formState: { errors }, setValue } = useForm<UpdateAthleteProfileFormInput>( {
        resolver: yupResolver( UpdateAthleteProfileSchema ),
    } );
        ...
        <>
            { !mutation.isError && !mutation.isPending && !countriesQuery.isLoading && !sportsQuery.isLoading && (
                <StyledForm onSubmit={ handleSubmit( onSubmitHandler ) }>
                    ...
                    { userType !== "SponsorCompany" && <TextField
                        { ...register( "lastName" ) }
                        label="Last name"
                        type="text"
...
export default UpdateAthleteProfileForm;
